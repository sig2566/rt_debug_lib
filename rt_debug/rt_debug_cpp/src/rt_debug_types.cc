...
void CTraceGroup::Reset(bool attach)
{
	...
	if(attach== false)
	{
		...
		{
			trace_fifo_.GetEntry(i)->sys_time.nf = NO_DATA_IND;  //Init trace entry indication
		}
		//Alloc Trace MemArea
...
void CTraceGroup::AddTrace(uint32_t trace_id, uint32_t line_num, GenSysTime *sys_time, uint64_t var0, uint64_t var1, uint64_t var2, uint64_t var3)
{
...
bool CTraceGroup::GetTraceEntry(char *trace_entry_str, timespec *linux_time, GenSysTime *sys_time)
{
	uint32_t i;
	...
	sys_time->nf=NO_DATA_IND;
	//Find  entry with minimal timestamp
	for(i=0; i< TRACE_ENTRIES_NUM; i++)
	{
		if(trace_fifo_.GetEntry(i)->sys_time.nf==NO_DATA_IND)
		{
			continue;
		}
	...
	*sys_time = trace_entry_ptr->sys_time;
	trace_entry_ptr->sys_time.nf= NO_DATA_IND;
	return true;
}
